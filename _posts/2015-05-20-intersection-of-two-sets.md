...
또한 Guava 와 마찬가지로 크기가 작은 Set 가 앞에 왔을때 성능이 더 좋다. 크기 비교를 하지 않고 단순 대입으로 진행해보니 조금 더 느린 41ms가 나왔다. `size()`를 여러번 호출하는건 성능에 별 영향을 끼치진 않는다.

## 결론

교집합을 구하는 방식을 기존 JDK 방식에서 NIH 방식으로 바꿔 **기존 대비 3.4배 성능 개선 효과**를 얻을 수 있었다.
