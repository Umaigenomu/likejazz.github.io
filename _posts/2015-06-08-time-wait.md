...
또한 클라이언트 소켓에 `SO_REUSEADDR`을 부여한다고 `TIME_WAIT`을 재사용할 수 있는게 아니다. 반드시 `tcp_tw_reuse` 커널 설정이 필요하다. 반대로 서버 소켓에서 `bind()`시에도 `tcp_tw_reuse` 설정을 했다고 즉시 바인딩 되지 않는다. 반드시 `SO_REUSEADDR` 옵션을 부여해야 한다. 이후에 다시 자세히 설명하도록 한다.
...
스택오버플로우의 답변[^fn-7]이나 그렇다는 논문[^fn-6]이 있는데 특히 메모리 점유 문제를 심각하게 얘기한다. 여기서 주의해야 할 점이 있다. `TIME_WAIT`으로 인한 성능 저하 논문은 1997년에 출판됐다. 지금은 2015년이다. 벌써 18년전 이야기다. 그 당시 서버 메모리는 512MB였고 지금 테스트를 진행하는 서버의 메모리는 64G다. 100배가 넘는다. 그런데 아직도 `TIME_WAIT` 때문에 메모리 점유가 심각하다고 말한다면 뭔가 이상하다.
...
서버가 할당하는 것은 포트가 아닌 **소켓**이며 서버 포트는 최초 `bind()`시 하나만 사용한다. 로컬 포트를 할당하는 것은 클라이언트이며, 클라이언트가 `connect()`시 로컬 포트를 임의로(ephemeral port) 바인딩하면서 서버의 소켓과 연결된다.
...
서버가 또 다른 서버에 클라이언트로써 접속하지만 않는다면 자신의 로컬 포트는 사용할 일이 없으며, 기본적으로 리눅스의 로컬 포트 수는 3만개로 설정 되어 있다. 만일 서버 투 서버로 1:1 대용량 접속이 발생할 경우 클라이언트의 최대 요청 수는 500 RPS 까지 가능하다. 이 수치를 넘어서지 않는다면 아무런 커널 설정도 변경할 필요가 없다.

### TIME_WAIT 재사용

그러나 `TIME_WAIT` 상태의 재사용이 필요한 경우가 있다. 클라이언트의 로컬 포트가 고갈될 경우이며 아래 3가지로 분류할 수 있다.
...
1번의 경우 클라이언트 입장에서는 서버에 남아 있는 `TIME_WAIT` 상태를 알 수 없다. 따라서 클라이언트는 계속해서 임의의 포트(ephemeral port)에 `SYN` 패킷을 내보낸다. 임의의 포트는 랜덤이 아닌 +1 증가하는 형태이므로 FIFO 기준을 따르게 된다. 서버쪽에는 `TIME_WAIT` 상태로 남아 있지만 `SYN`을 수신하면 소켓을 재사용하게 된다. 양쪽 모두 별도 설정은 필요 없다.
...
분명 최신 OS에서는 별도 설정 없이, 문제 없이 로컬 포트가 할당되는데 오래된 OS에서 오류가 발생한 것이다. 오류는 `Cannot assign requested address`로 로컬 포트 고갈로 발생하던 2번과 동일하며 서버에 `SYN` 패킷 조차 전달하지 못한다. 클라이언트는 서버의 `TIME_WAIT` 상태를 알 수 없으므로 문제가 없어야 하나 부하가 매우 높을때 간헐적으로 소켓 할당이 안되는 걸로 봐서 오래된 OS의 특징 또는 버그로 추정된다. 

이 경우 해결책은 2번과 마찬가지로 `net.ipv4.tcp_tw_reuse`를 설정하는 것으로 간단히 해결 가능하다.
...
`TIME_WAIT`을 재활용하는 `net.ipv4.tcp_tw_recycle` 옵션이 있지만 일반적으로 NAT 환경에서 문제가 있다[^fn-2]고 알려져 있다. 서비스 환경에서 서버가 로드 밸런스 뒤에 위치한다면 패킷 드롭이 발생할 수 있으므로 사용해선 안된다.
