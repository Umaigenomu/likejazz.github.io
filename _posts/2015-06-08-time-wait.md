...
참고로 `SO_REUSEADDR`가 `tcp_tw_reuse`와 동일한 역할을 한다는 문서가 있는데 **잘못된 정보**다. `SO_REUSEADDR`은 소켓 바인딩시 해당 포트의 네트워크 상태를 커널에서 즉시 돌려 받는 것(**주소 재사용**)이고, `tcp_tw_reuse`는 새로운 연결에 `TIME_WAIT` 상태를 재사용하는 것(**상태 재사용**)이다. 전자는 서버의 역할이고, 후자는 클라이언트의 역할로 전혀 다르다. 

클라이언트 소켓에 `SO_REUSEADDR`을 부여한다고 `TIME_WAIT`을 재사용할 수 있는게 아니다. `tcp_tw_reuse` 커널 설정이 필요하다. 반대로 서버 소켓에서 `bind()`시 `tcp_tw_reuse` 설정을 했다고 바인딩 되지 않는다. `SO_REUSEADDR` 옵션을 부여해야 한다.

이후에 다시 자세히 설명하도록 한다.

### 자원 점유

그렇다면 과연 다수의 `TIME_WAIT`은 시스템 성능 저하를 가져올까. 
...
1. `tcpdump`에서 보이지 않을리 없다. 서버는 바인딩된 포트만 사용해서 패킷을 주고 받음을 `tcpdump`로 분명히 확인할 수 있다.
...
서버가 또 다른 서버에 클라이언트로써 접속하지만 않는다면 자신의 로컬 포트는 사용할 일이 없으며, 리눅스의 로컬 포트 범위는 3만개 정도로 설정되어 있다.
...
만일 서버 투 서버로 1:1 대용량 접속이 발생할 경우 클라이언트의 최대 요청 수는 500 RPS 까지 가능하다. 이 수치를 넘어서지만 않는다면 아무런 커널 설정도 변경할 필요가 없다.
...
2번은 `Cannot assign requested address` 오류가 발생하며 더 이상 접속할 수 없다. 소켓은 \<protocol\>, \<src addr\>, \<src port\>, \<dest addr\>, \<dest port\> 5개 값으로 구성되며 로컬 포트가 고갈되면 더 이상 유니크한 값을 만들어 낼 수 없다. 클라이언트의 `net.ipv4.tcp_tw_reuse` 옵션을 설정하여 기존 클라이언트에 `TIME_WAIT` 상태로 남아 있던 소켓을 재사용해야 한다.
...
3번은 문제가 없다. 앞서 언급했듯 소켓은 5개 유니크 값이며 맨 마지막이 \<dest port\> 이다. 이 말은 포트가 다를 경우 다시 그만큼의 소켓 쌍을 만들어낼 수 있음을 뜻한다. 재사용이 필요 없다.
...
최신 OS에서는 별도 설정 없이, 문제 없이 로컬 포트가 할당되는데 오래된 OS에서 `Cannot assign requested address` 오류가 발생한다. 로컬 포트 고갈로 발생하던 2번과 동일한 오류이며 네트워크 전체가 매우 느려진다. 다른 포트로 연결은 가능하나 매우 느려진 상태에서 정상적인 진행이 힘들 정도다. 부하가 매우 높을때만 간헐적으로 발생하는 걸로 봐서 남아 있는 포트를 스캔 하느라 느려지는 걸로 보인다.[^fn-2] 이미 로컬 포트를 다 사용한 상태에서 매 번 풀스캔에 리소스를 허비하는 것이다.

이 경우 해결책은 2번과 마찬가지로 클라이언트의 `net.ipv4.tcp_tw_reuse`를 명시적으로 설정하면 간단히 해결된다. 커널이 남아 있는 포트를 매 번 스캔할 필요 없이 항상 재사용으로 빠르게 문제 해결이 가능해진다.
...
`TIME_WAIT`을 재활용하는 `net.ipv4.tcp_tw_recycle` 옵션도 있지만 일반적으로 NAT 환경에서 문제가 있다[^fn-2]고 알려져 있다. 서버가 로드 밸런스 뒤에 위치하는 서비스 환경에선 패킷 드롭이 발생할 수 있으므로 사용해선 안된다.
...
1. `l_onoff = 1`, `l_linger = non-zero` : 명시된 시간(초)동안 정상적으로 진행하며 그 이후에는 비정상 종료 처리한다.
1. `l_onoff = 0` : 정상적인 4-way handshake 종료 과정을 진행하며 소켓의 기본값이다.

`RST` 는 비정상 종료시 보내는 패킷이며 수신한 상대방은 `Connection reset by peer` 오류가 나게 된다. 양쪽 모두 바로 연결이 끊어지며, 양쪽 모두 `TIME_WAIT` 상태가 남지 않는다.

가장 빠르고 깔끔하게 연결이 끊어진다는 점에서 HAProxy, nginx 등에서도 사용되고 있지만 `RST`는 이제 더 이상 연락하지 않는 일방적인 이별 통보로써 또다른 side effects를 야기할 수 있다. 또한 양쪽 모두 `TIME_WAIT`을 남기지 않기 때문에 패킷의 오동작이 우려된다.
...
- 다수의 `TIME_WAIT`이 서버 성능을 저하시킨다는 논문[^fn-6]은 1997년에 출판됐다. 지금은 2015년이다. 그 당시 서버는 512MB 메모리였고, 지금 테스트를 진행하는 서버는 64G 메모리를 갖고 있다.
- `TIME_WAIT`을 재사용하는 `tcp_tw_reuse` 옵션이 필요한 것은 클라이언트 뿐이다. 서버는 아무 소용이 없다.
- `tcp_tw_reuse`는 `SO_REUSEADDR` 옵션과 다르다.
- 서버가 클라이언트를 `accept()` 할때 사용하는 것은 소켓 뿐이다. 로컬 포트가 아니다. 서버는 단 하나의 포트만 있으면 된다.
- 클라이언트는 `connect()`시 임의의 로컬 포트(ephemeral port)를 할당한다.
- 소켓의 최대 갯수는 65K가 아니다. 소켓은 \<protocol\>, \<src addr\>, \<src port\>, \<dest addr\>, \<dest port\> 이 5개의 값으로 유니크하게 구성된다. 서버의 포트는 하나이고 클라이언트의 포트는 계속 변한다. 만약 서버의  포트가 추가되거나 클라이언트의 IP가 추가될 경우 다시 그 만큼의 새로운 쌍을 생성할 수 있다.
