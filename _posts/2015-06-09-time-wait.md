...
소켓의 수는 리눅스 파일 디스크립터만큼 생성할 수 있다.

{% highlight bash %}
$ cat /procsys/fs/file-max
{% endhighlight %}
...
가장 빠르고 깔끔하게 연결이 끊어진다는 점에서 유용할 수 있지만 문제는 비정상 종료라는 점이다. `RST`는 더 이상 연락하지 말자는 일방적인 이별 통보로, 또다른 side effects를 야기할 수 있다. 또한 양쪽 모두에 `TIME_WAIT`을 남기지 않기 때문에 패킷의 오동작을 막아줄 장치가 없다.
...
- 소켓의 최대 갯수는 65,535개가 아니다. 소켓은 \<protocol\>, \<src addr\>, \<src port\>, \<dest addr\>, \<dest port\> 이 5개의 값으로 유니크하게 구성된다. 서버의 포트는 하나이고 클라이언트의 포트는 계속 변한다. 만약 서버의  포트가 추가되거나 클라이언트의 IP가 추가될 경우 다시 그 만큼의 새로운 쌍을 생성할 수 있다.
