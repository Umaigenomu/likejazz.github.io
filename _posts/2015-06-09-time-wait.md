...
`net.ipv4.tcp_fin_timeout`을 설정하면 `TIME_WAIT`의 타임아웃을 변경할 수 있다는 내용이 있는데 앞서 소개했듯 **잘못된 정보**다. 커널 헤더에 `TIME_WAIT`은 `TCP_TIMEWAIT_LEN` 이라는 상수로 60초 하드 코딩 되어 있으며 변경할 수 없다. `tcp_fin_timeout`은 `FIN_WAIT2`의 대기 시간이며 `TIME_WAIT`과는 무관하다.
