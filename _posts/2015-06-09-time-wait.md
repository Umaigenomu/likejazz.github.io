...
`net.ipv4.tcp_fin_timeout`을 설정하면 `TIME_WAIT`의 타임아웃을 변경할 수 있다는 내용이 많은데 이는 **잘못된 정보**다. `TIME_WAIT`의 타임아웃 정보는 커널 헤더 `include/net/tcp.h` 에 하드 코딩 되어 있으며 변경이 불가능하다.
...
`net.ipv4.tcp_fin_timeout`을 설정하면 `TIME_WAIT`의 타임아웃을 변경할 수 있다는 내용이 있는데 앞서 소개했듯 **잘못된 정보**다. 커널 헤더에 `TIME_WAIT`은 `TCP_TIMEWAIT_LEN` 이라는 상수로 60초 하드코딩 되어 있으며 변경할 수 없다. `tcp_fin_timeout`은 `FIN_WAIT2`의 대기 시간이며 `TIME_WAIT`과는 무관하다.

`FIN_WAIT2`는 Active Close를 했는데 Passive Close 쪽에서 `close()`를 처리하지 못하고 `CLOSE_WAIT` 상태에 빠졌을때를 말한다. 즉, 상대방에 문제가 있는 상태로, 일정 시간 기다려 주는게 좋다. 최근 리눅스에서 기본값은 마찬가지로 60초다.

`TIME_WAIT`의 대기 시간이 `tcp_fin_timeout` 설정에 영향을 받는다는 잘못된 오해는 오히려 그 반대인 `TCP_TIMEWAIT_LEN` 상수값이 `FIN_WAIT2`의 대기시간에 영향을 끼치기 때문에 잘못 와전된듯 하다. 무슨 말인지는 아래 커널 소스에서 확인할 수 있다.
...
먼저 `tcp.h` 코드를 보면 `tcp_fin_timeout` 설정은 당연히 링거 옵션에 영향을 받는다. 링거에 대해선 이후에 다시 설명한다. 

그리고 `tcp.c` 코드에는 `FIN_WAIT2` 상태일때 대기 시간 설정 로직이 있다. `tcp_fin_time()`을 호출한 `tmo` 값. 즉, `tcp_fin_timeout` 값이 만약 `TCP_TIMEWAIT_LEN` 상수보다 크다면 그 시간을 뺀만큼만 기다리게 된다. 즉, 리눅스 커널에서는 `TIME_WAIT`의 대기 시간이 `FIN_WAIT2`의 대기 시간 설정에 영향을 끼치는 것이다. `TIME_WAIT`은 60초로 설정되어 있다. 그러므로 만약 `tcp_fin_timeout`을 70으로 설정한다면 60초를 뺀 10초만 기다리게 될 것이다. 

실제로 그렇게 동작하는지 확인해본다.

{% highlight bash %}
$ echo 70 > /proc/sys/net/ipv4/tcp_fin_timeout
$ while true; do ss -tanpo | grep 5000; sleep 1; done
FIN-WAIT-2 0      0              10.51.31.139:1046         10.41.147.237:5000   timer:(keepalive,9.836ms,0)
{% endhighlight %}

70으로 설정했을때 정확히 `TIME_WAIT`의 대기 시간을 뺀 10초의 타임아웃이 설정됨을 확인할 수 있다. 만약 60보다 작은 값일 경우 그냥 그 시간만큼 기다리게 될 것이다. (i.e. 20으로 설정하면 20)
...
- `TIME_WAIT`의 타임아웃은 60초로 하드코딩되어 있다. 설정할 수 없다.
...
- 소켓의 최대 갯수는 65,535개가 아니다. 소켓은 *\<protocol\>, \<src addr\>, \<src port\>, \<dest addr\>, \<dest port\>* 5개의 값으로 유니크하게 구성되며, 서버 포트가 추가되거나 클라이언트의 IP가 추가될 경우 그 만큼의 새로운 쌍을 생성할 수 있다.
