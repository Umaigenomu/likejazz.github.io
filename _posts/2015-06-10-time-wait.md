...
또한 양쪽 모두 `TIME_WAIT`이 남는 재미난 현상도 발생할 수 있다. 양쪽 모두 동일 시점에 Active Close를 시도할 경우다.
...
만약 서버가 로컬 포트를 사용하고 로컬 포트는 단 하나의 소켓에만 바인딩된다고 가정하면,

1. 지금 처럼 9만개 가까운 `TIME_WAIT`을 만들어 낼 수 없다. 로컬 포트는 (2^16)-1 = 65,535개가 최대다.
1. OS 몰래 비밀 포트라도 여는게 아니라면 `tcpdump`에 보이지 않을리 없다. 서버는 최초 바인딩된 포트만 사용해서 패킷을 주고 받음을 분명히 확인할 수 있다.
...
이 경우 해결책은 2번과 마찬가지로 클라이언트의 `net.ipv4.tcp_tw_reuse`를 명시적으로 선언하면 간단히 해결된다. 커널이 비어 있는 포트를 매 번 스캔할 필요 없이 항상 재사용하므로 빠르게 처리가 가능하다.
...
두 번째 문제는, 새로운 커넥션이 `SYN`을 보내면 타임스탬프를 비교해 무시하고 `RST`는 보내지 않을 것이다. 그러는 사이 서버의 `FIN`이 재전송된다. 그러면 `SYN_SENT` 상태에 있던 클라이언트는 `RST`를 보낸다. 이제 서버가 `LAST_ACK` 상태를 빠져나온다. 그러는 사이 클라이언트는 `ACK`을 받지 못한 상태이므로 1초 후 `SYN`을 재전송한다. 이제 둘은 정상적으로 `ESTABLISHED` 된다.[^fn-2]

타임스탬프 덕분에 오류를 내며 빠져 나오지 않고 않고 정상적으로 연결됐다. 약간의 딜레이만 발생했을 뿐이다.
...
`TIME_WAIT`을 재활용하는데는 성능이 매우 좋은 `net.ipv4.tcp_tw_recycle` 옵션이 있다. 그러나 NAT 환경에서 문제가 있다[^fn-2]고 알려져 있다. 서버가 로드 밸런스 뒤에 위치하는 서비스 환경에선 [패킷 드롭이 발생](http://tagnee.tistory.com/22)할 수 있으므로 **사용해선 안된다**. 아쉽게도 서버/클라이언트가 1:1로 직접 연결된 경우가 아니라면 사용해선 안된다.
...
`FIN_WAIT2`에서 `ACK`을 받지 못하면 소켓은 곧바로 종료된다. 따라서 리눅스 커널은 `FIN_WAIT2`를 `TIME_WAIT`처럼 활용하고 있다. `TIME_WAIT`은 패킷의 오동작을 막아주는 역할을 하는데 `FIN_WAIT2` / timewait 상태는 `tcp_tw_recycle` 처리 포함, 일부 잘못된 패킷 처리 로직이 상단에 추가된 형태로 사실상 동일한 역할을 한다.
...
// 상단에 몇 가지 패킷 처리만 추가 되어 동일하게 동작한다.
...
        }
...
1. `l_onoff = 1`, `l_linger = non-zero` : 명시된 시간(초)동안 정상적으로 진행하며 그 이후에는 비정상 종료 처리한다. 만일 버퍼에 전송되지 못한 메시지가 남아 있다면 명시된 시간 동안은 어플리케이션이 `close()`를 진행하지 못하고 최종적으로 `RST`를 보내기 위해 대기하기 때문에 지연 현상이 발생한다. 따라서 추천하지 않는다.
...
[^fn-8]: <https://www.facebook.com/likejazz/posts/10153290132235837?comment_id=10153292111270837&offset=0&total_comments=4&comment_tracking=%7B%22tn%22%3A%22R0%22%7D&pnref=story>
