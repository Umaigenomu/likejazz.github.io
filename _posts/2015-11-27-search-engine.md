...
1. 모든 문서는 영어로  
원어민이 아닌 이상 영어는 누구나 힘들다. 하지만 좋은 소프트웨어가 언어 문제로 사장되는건 안타까운 일이다. 언어의 제약 없이 전세계에 지식을 공유하기 위해선 모든 코드와 주석, 문서를 영어로 적는 노력이 필요하다.
2. C++ 컨버팅  
파이썬은 최고의 프로토타이핑 도구이지만 IIR의 기본 컨셉을 밑바닥 부터 구현하고 고성능을 내기 위해서는 좀 더 로우 레벨의 언어가 필요했다. 추후에 C++로 처음부터 새로 작성해보면 좋을 것 같다.
3. 훌륭한 GitHub  
