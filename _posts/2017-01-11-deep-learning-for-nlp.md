...
<div class="message">
<p>2012년 이미지넷에서 토론토 대학의 알렉스 크리제브스키가 들고 나온 딥러닝 '알렉스넷(Alexnet)'은 놀랄만한 정확도를 보여주며 우승했다. 이후 불과 수 년 사이 모든 참가자들이 딥러닝으로 들고 나올 정도로 딥러닝은 비전 인식에 획기적인 변화를 주도 하고 있다. (아래 그래프 참조)</p>
...
![IMAGENET Accuracy Rate](http://postfiles2.naver.net/20160321_257/kor_nvidia_1458547387540mHprX_PNG/%B1%D7%B8%B22.png?type=w2)
...
가뜩이나 책 내용이 복잡한 수식으로 가득찬 어려운 이론서인데, 번역 또한 이해하고 풀어쓴게 아니라 원문의 거의 직역에 가깝기 때문에 한국어로 읽었을때 도무지 무슨 말인지 이해할 수가 없다. 차라리 원서를 보는 편이 이해가 더 빠르다. 아래에 소개하는 '텐서플로 첫걸음'이 완벽히 이해하고 번역한 모범 사례를 보여준다면 이 책은 이해하지 못한 상태에서 번역하면 어떻게 되는지를 보여준 나쁜 사례. 안타까운 부분이다.
