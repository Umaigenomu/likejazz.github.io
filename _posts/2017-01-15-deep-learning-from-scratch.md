...
이전에 가중치 매개 변수에 대한 손실함수 기울기는 수치 미분을 사용해 구하고 기울기의 10%(학습률 0.1) 만큼 이동하도록 구현했음

수치 미분의 $${\delta}$$ 값은 0.0001로 지정. 범위가 클수록 해석적 미분의 값과 차이
...
신경망이 깊을때 0 ~ 1 활성화 함수를 통한 기울기는 매우 작은 값이고 역전파<sup>Backpropagation</sup>중에 배가되어 깊은 망에서 '소실'되는 효과를 가져온다. ReLU와 LSTM 아키텍처를 사용하여 해결하는 방법이 일반적이다.
...
fc와 ReLU 사이 BN 레이어 삽입. 데이타 분포 평균 0, 분산 1이 되도록 정규화. [논문 해석 참고][13] 학습 속도를 높이는 효과
...
경사 하강법의 기울기 방향 변화(가중치 업데이트):

$$ w_i \leftarrow w_i-\eta\frac{\partial E}{\partial w_i}$$

경사 하강법에 가중치 감소를 적용한 새로운 비용 함수 적용:[^14]
...
검증 데이타<sup>Validation Set</sup>  
하이퍼 파라미터 전용 확인 데이타  
본문의 경우 learning rate: 0.001 ~ 0.01, weight decay: 10^-8 ~ 10^-6
TODO: weight decay 값이 $${\lambda}$$ 확인
