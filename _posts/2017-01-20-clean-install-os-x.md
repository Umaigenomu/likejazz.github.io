...
- 관심 있는 링크는 항상 [Delicious](http://likejazz.com/post/108592366618/delicious-com)에 저장해 왔으나 [이제는 삼류 서비스의 행태](http://likejazz.com/post/143509552915/leaving-deliciouscom)를 보여 아쉽다. 10년 넘게 사용해왔으나 점점 **Pocket** 사용 시간이 늘고 있다.
...
- **Wake Up Time - Alarm Clock** 아침마다 이걸로 일어난다.
- **Near Lock(유료)** 맥북 잠금/해제를 관리한다. 10m로 설정해서 아이폰이 맥북에서 10m 이상 떨어지면 자동으로 잠기고, 이내로 들어서면 해제된다. 백그라운드로 실행되는 아이폰 앱이 유료다.
...
- **VMware Fusion(유료)** 을 줄곧 사용해오다 비싼 가격으로 Virtual Box로 이동하려 했으나 기존 이미지 파일을 복구하려면 방법이 없다. 윈도우를 다시 설치할 엄두가 나지 않아 어쩔 수 없이 계속 사용하고 있다. 이미 윈도우 이미지 파일 크기가 60G를 넘어 섰다.
- ~~**EZPlus for Mac** 신한 은행 인터넷 뱅킹 프로그램. 그나마 우리나라 은행에서 맥 지원이 되는 것에 감사하며 사용 중~~ 주 은행은 **카카오 뱅크**를 사용 중이다.

### 커뮤니케이션
- **카카오톡** 메신저 설치
- **Slack** 팀 업무용 커뮤니케이션 채널로 사용하는데 너무 느려서 최근에는 사용 빈도가 줄고 있다.

### 문서
- **구글 드라이브**를 주로 사용하며 그외 문서는 **깃헙 위키**로 작성 한다. 출력을 위해서는 여전히 **MS 오피스**를 활용한다.
- **Markoff** 굳이 서버를 구동하지 않고도 깃헙 위키를 미리 보기할 수 있어 편하다.
...
### 이미지, 비디오
- **Xee³(유료)** 빠른 속도 뿐만 아니라 편리한 브라우징이 돋보이는 최고의 이미지 뷰어인데 굳이 유료로 구매할 정도는 아니어서 유료 출시 직전의 무료 버전인 2.2를 구해서 이용한다.
- Pixelmator를 사용해오다 요즘은 다시 **포토샵(유료)** 사용 중. 간단한 편집시에는 좀 더 가벼운 도구를 사용하고 수정이 많이 필요한 경우에만 가끔 실행한다.
- **Sketch(유료)** 프로그래밍 입문자 부터 전문가까지 모두 파이썬을 사용하는 것처럼 디자인 입문자 부터 전문가까지 모두 Sketch를 사용하는듯 하다. 내 경우 주로 프로토타이핑 용도로 활용 하고 있다.
- 동영상 플레이어로 **IINA**를 이용한다. Swift로 만든 터치바도 지원하는 깔끔한 동영상 플레이어다.
...
- **AirFlow(유료)** 크롬 캐스트로 동영상을 캐스팅한다. TV로 영화를 볼때 사용한다.
...
- 개발 관련 디렉토리 생성. 지난 몇 년간 항상 `~/workspace`을 만들어 사용해오고 있으며, 요즘은 깃헙 프로젝트가 대부분이라 하위에 각 사이트별 URL 기준 디렉토리를 만들어 사용 중이다. 자바에서 net.daum 으로 시작하는 import 를 만드는 것과 비슷.
...
- 코딩을 할때는 항상 JetBrains의 IDE를 사용하지만 문서를 작성하거나 간단한 코드에는 여전히 **[Sublime Text 3](http://likejazz.com/post/102824813705/sublime-text)** 를 사용한다. IDE와 코드 편집기의 중간 정도로 **Visual Studio Code**를 사용한다. vim도 이용하지만 역시 GUI와 마우스를 병행할때 최고의 생산성을 발휘한다.
...
- **Docker** '개발-테스트-서비스' 설정/설치는 항상 고민거리인데 도커는 그 고민을 깔끔하게 해결해줬다. 사실상 OS까지 함께 설치하는 컨테이너 개념은 매우 편하다. 게다가 맥에서도 작년부터 native hypervisor를 지원해서 virtualbox를 사용해야 하는 불편함이 말끔히 해결됐다. 함께 제공되는 GUI 관리툴 **Kitematic**도 너무 편리하며, 메뉴바에서 간단히 상태를 확인할 수 있는 **Captain**도 훌륭하다.
