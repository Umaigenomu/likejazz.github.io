...
- Night Shift를 사용하며, 스케쥴은 Sunset to Sunrise로 사용한다.
- 터치바 인터페이스는 거의 쓰지 않는다. 나비 키보드와 함께 신형 맥북의 새로운 기능은 모두 실패작으로 보인다.

### 인터넷
- 브라우저는 **Google Chrome**을 주로 쓴다.
...
- 번역 도구로 **Google Translate**를 이용한다. 딥러닝이 적용된 구글 번역의 품질은 매우 훌륭하다. 최근에는 **[카카오 번역기](https://translate.kakao.com/)**를 주로 이용하며, 카톡에 친구로 등록하면 훨씬 더 편리하게 이용할 수 있다.
- 당연히 **Save to Pocket** 익스텐션도 사용한다.

### 생산성
- 버전 관리를 위해 앱은 앱스토어나 **Cask**를 통해 설치한다. 앱스토어 쪽이 좀 더 접근성이 좋고 편리하다.
...
- **Logitech Options** 로지텍 블루투스 마우스를 사용 중이며, 별도 키 맵핑을 위해 필요하다.
...
### 문서
- 이 문서를 호스팅하는 공간은 **GitHub Pages**다. 수년간 최고의 지식 저장소로 활용하고 있으며, **MathJax**를 이용한 수식 지원도 최고다. 깃헙 위키의 경우 수식을 입력할 수 있는 방법이 마땅찮아서 어쩔 수 없이 모두 깃헙 페이지로 이관했다.
...
- **Xcode**를 거의 사용하진 않지만 **Command Line Tools**를 위해 여전히 필수로 설치한다. LLVM을 비롯한 각종 개발 도구를 최신 버전으로 업데이트 하기 위해선 필수다.
...
- dotfiles 설정이 번거로운데 깃헙에 공개 형태로 출판도 해보았으나 여전히 관리하기 번거롭다. 새 노트북을 셋팅할때는 Dropbox에 보관했다가 그걸 다시 새 노트북에 셋팅한다. `/etc/hosts`, `.vimrc`, `.zshrc` 등등.
...
### 자료
개인 자료의 경우 사진과 비디오는 이미 **Google Photos**를 활용하고 그외 블로그와 기타 이미지 호스팅 용도로 **GitHub**의 Issues를 활용한다. 비디오 중 공개적인건 **YouTube**에, 음악은 따로 다운받지 않고 **멜론** 스트리밍을 사용하고, 문서는 **Dropbox**와 **iCloud Drive**에 적절히 나눠 저장하고, 오피스 문서는 **OneDrive**에, 정리가 필요한 문서는 **GitHub** 위키에 마크다운으로 작성한다. 공개적인 코드는 공개 **GitHub**에, 회사 업무용은 사내 **GitHub Enterprise**에 둔다.
