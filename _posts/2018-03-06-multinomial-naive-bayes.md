...
각 조건부 확률<sup>likelihood</sup>의 로그 합이며, 이를 백분율로 환산하여 보여준다. 일반적인 베이즈 룰 수식과 달리 분모, 즉 **증거<sup>evidence</sup>는 사용하지 않는데**, 이는 모든 클래스에 대해 항상 같은 값을 갖기 때문에 argmax에 아무런 영향을 끼치지 않기 때문이다. 실제로 나이브 베이즈가 성공적인 이유는 사후 확률의 우수함이 아니라 $$ argmax_y{P(Y \rvert X)} $$ 의 우수함 때문이라 말하며(Domingos, Pazzani, 1997) 확률 추정은 좋지 않지만 성능이 좋은 이유를 설명한다.
