...
<div class="message">
LSTM은 RNN의 핵심을 이루는 모델이다. 그러나 개념적으로 잘못 이해하고 있는 경우가 많은데, 원리를 정확히 이해하고 제대로 숙지하는 것은 항상 중요한 만큼 여기서는 원리와 수식을 이해하고 직접 계산을 통해 결과값을 검증해보도록 한다.
</div>
...
프랑소와 숄레는 LSTM을 설명하면서 *새로운 이동 상태 계산*으로 표현했다.
...
마지막 $$h_9$$의 값이 Keras 계산 결과와 동일하며, 따라서 우리가 위키피디어 수식을 기준으로 구현한 NumPy 코드는 Keras 구현과 동일함을 확인할 수 있다.
