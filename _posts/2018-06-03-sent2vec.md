...
#### negative sampling
1M 단어의 softmax를 구하는 것은 매우 연산 비용이 높은 일이기 때문에 이를 극복하기 위한 몇 가지 기법이 있다. softmax-based approaches로는 hierarchical softmax가 대표적이며, 여기서는 sampling-based approaches인 negative sampling를 살펴보도록 한다. negative sampling은 target에 해당하는 label=true는 시그모이드가 1이 되도록 학습하고 나머지 negative samples를 추출하여 시그모이드가 0이 되도록 빠르게 학습하는 기법이다.
```c++
...
context window는 고정 사이즈가 아니다. `-ws`는 max window 값으로 uniform sampling된 window 사이즈가 랜덤하게 정해진다. dynamic context window 기법으로 아래와 같은 형태로 실행된다. CBOW의 경우 target(center) 이전과 이후 윈도우 모두에 해당하기 때문에 `-ws`가 5인 경우 최대 윈도우 사이즈는 아래와 같이 10이 된다.
```
token: 거래목적 wid: 1730
token: 또는 wid: -1
...
#### character n-grams
기본적으로 fastText CBOW구현은 기존의 word2vec과 동일하다. 그러나 중요한 차이점으로 character n-grams 지원이 있는데, 이는 char 단위로 subwords를 추출하여 학습에 참여하고 앞서 `-ws 5`일때 bow size가 30 가까이 늘어나기도 한다. 아래 결과와 같이 영어에서는 다양한 변형을 학습할 수 있는 장점이 있으나 한글의 경우 조합 가능한 단어의 수가 매우 많기 때문에 이 기법은 적절해 보이진 않는다. `-minn 0 -maxn 0`으로 비활성화 할 수 있다.
...
### Sent2Vec
Sent2Vec은 단어가 아닌 문장 단위의 학습 의미를 살리기 위해 CBOW를 변형한 몇 가지 기법을 도입했다. 먼저 context window 크기를 임의로 정하는 것과 달리 context window가 문장 전체를 커버하도록 자동으로 설정한다. 즉, 전체 문장이 7개 단어로 구성되어 있다면 context window의 크기도 7이 된다. context window의 크기는 문장의 단어 수와 일치한다. 이외에도 논문에는 중요한 차이점으로 크게 두 가지를 언급한다.[^fn-1]
...
2. CBOW는 **dynamic context window**를 사용한다. dynamic context window를 사용하는 것은 포커스 단어 $$w$$에서 윈도우 크기로 나눈 거리만큼 weighting 하는 것과 같다. (Levy et al., 2015) 이는 prediction task를 local<sup>국소화</sup>로 만들며 문장의 모든 n-grams를 조합하여 학습 하고자 하는 우리의 목표에 반한다.
