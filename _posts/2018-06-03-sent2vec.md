...
CBOW는 context가 target(center)을 갖도록 학습한다. skipgram과 달리 전체 평균을 입력값으로 하기 때문에 단어 위치를 보지 않는 bag of words 형태로, 때문에 학습 속도는 빠르지만 일반적으로 skipgram에 비해 성능은 떨어지는 것으로 알려져 있다. 다만, fastText 구현에는 평균이 아닌 각각 개별 학습을 진행하는 형태로 구현되어 있으므로 추가 확인이 필요하다. 에러 함수의 미분까지 간단히 코드로 표현해보면 아래와 같다.
...
![](https://user-images.githubusercontent.com/1250095/41037125-4659a8e6-69cd-11e8-958d-5f93a1d7a8bd.png)[^fn-5]
