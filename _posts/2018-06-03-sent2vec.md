...
word2vec의 핵심 알고리즘을 직접 구현한 코드는 [word2vec.py](https://github.com/likejazz/jupyter-notebooks/blob/master/deep-learning/word2vec.py)에서 확인할 수 있다. 그러나, 이 코드로는 대형 코퍼스를 학습할 수는 없다. 대형 뉴스 코퍼스의 경우 단어 수가 1M에 달하기 때문인데, 따라서 학습 속도를 높이기 위해 다양한 트릭을 사용한다.

#### subsampling
대표적인 기법이 서브 샘플링으로, 빈도 수 높은 단어를 학습에서 제외하여 학습 속도를 높이고 불용어 제거와 유사한 효과를 갖는다.[^fn-3] 랜덤 확률로 학습에서 배제하는 방법이며, Mikolov의 2013년 첫 논문에는 $$p_{disc}(w)=1-\sqrt{\frac{t}{f}}$$로 표기되어 있으며 이후 word2vec, fastText에 이르러 최종 구현은 아래 수식을 사용한다.
...
#### negative sampling
1M 단어의 softmax를 구하는 것은 매우 연산 비용이 높은 일이기 때문에 이를 극복하기 위한 몇 가지 기법이 등장했다. softmax-based approaches로는 hierarchical softmax가 대표적이며, 여기서는 sampling-based approaches인 negative sampling를 살펴보도록 한다. negative sampling은 target에 해당하는 label=true는 시그모이드가 1이 되도록 학습하고 나머지 negative samples를 추출하여 시그모이드가 0이 되도록 빠르게 학습하는 기법이다.
```c++
...
```
일반적으로 CBOW는 5, skipgram은 10을 사용하며, `-neg` 옵션으로 정할 수 있다. 대형 코퍼스 일수록 이 값을 더 낮출 수 있으며, 논문에는 아래와 같이 기술되어 있다.[^fn-4]
...
context window 사이즈는 고정된 값을 부여하지만 실제로는 고정 사이즈로 동작하지 않는다. `-ws`는 max window 값으로 uniform sampling된 window 사이즈가 랜덤하게 정해진다. dynamic context window 기법으로 아래와 같은 형태로 실행된다. CBOW의 경우 target(center) 이전과 이후 윈도우 모두에 해당하기 때문에 `-ws`가 5인 경우 최대 윈도우 사이즈는 아래와 같이 10이 된다.
```
token: 거래목적 wid: 1730
token: 또는 wid: -1
...
#### character n-grams
기본적으로 fastText CBOW구현은 기존의 word2vec과 동일하다. 그러나 중요한 차이점으로 character n-grams 지원이 있는데, 이는 char 단위로 subwords를 추출하여 학습에 참여하는 기법이다. 모든 subwords 학습에 참여시키기 때문에 앞서 `-ws 5`일때 bow size가 30 가까이 늘어나기도 한다. 아래 결과와 같이 영어에서는 다양한 변형을 학습할 수 있는 장점이 있으나 한글의 경우 조합 가능한 문자의 수가 매우 많기 때문에 이 기법은 적절해 보이진 않는다. `-minn 0 -maxn 0`으로 비활성화 할 수 있다.
...
이제 Sent2Vec이 기존 CBOW와 차이점을 살펴보도록 한다.

### Sent2Vec
Sent2Vec은 단어가 아닌 문장 단위의 학습 의미를 살리기 위해 CBOW를 변형한 몇 가지 기법을 도입했다. 먼저 context window 크기를 임의로 정한 것과 달리 context window가 문장 전체를 커버하도록 크기를 자동으로 설정한다. 즉, 전체 문장이 7개 단어로 구성되어 있다면 context window의 크기도 7이 된다. context window의 크기는 문장의 단어 수와 일치한다. 이외에도 논문에는 중요한 차이점으로 크게 두 가지를 언급한다.[^fn-1]
...
따라서 Sent2Vec은 subsampling을 하지 않으며, dynamic context window도 사용하지 않는다. context window 크기는 문장의 전체 길이로 고정한다. 따라서 옵션 `-t`와 `-ws`는 무시된다. 혼동을 막기 위해 아예 제거되어야 할 옵션인데 남아 있는 것은 일종의 버그로 추측된다. 이외에도 ngram 확장의 오버피팅을 막기 위해 `-dropoutK`로 ngram 생성에 참여하는 토큰을 랜덤하게 배제한다. 다만, 이 옵션은 `-wordNgrams`을 2이상 지정하지 않을 경우 해당 사항이 없다.
...
### fastText 구현
#### Pre-processing
`word2int_[h]`의 값 `wid`는 단어의 출현 빈도 순으로 정렬된다. 즉, 1일때 가장 많이 노출된 단어이며 CBOW에서 subsampling 확률이 가장 낮다. 뒷 부분 단어들은 앞서 subsampling 수식에 따라 확률이 1이 넘기 때문에 항상 학습에 참여한다. 함수 `threshold()`가 실행될때 빈도순 정렬이 이뤄지며 이 함수는 또한 `-minCount` 이내인 토큰을 삭제하는 역할을 담당 하기도 한다.

#### 학습
`-lrUpdateRate`는 학습 진행 상황을 표시할 토큰의 단위다. 디폴트는 100이며, 100개의 토큰이 학습될 때 마다 진행 상황이 갱신된다. 로그 파일이 상당히 커질 수 있으므로, 대형 코퍼스인 경우 1000이상으로 설정해도 충분할 것 같다.
...
현재 pypi에서 제공되는 파이썬 바인딩은 2가지 이고, 페북 공식 버전(fastText)은 pybind11 C++ 버전, pyfasttext는 Cython C 버전이라는 차이가 있다. 구현에 큰 차이는 없으므로 만약 성능 이슈가 있다면 pybind11 또는 Cython 성능이 문제일 것이다. 예전에 [실험 결과](http://docs.likejazz.com/python-numpy-extensions/), Cython의 성능이 좀 더 좋은 것으로 나온적이 있다.
