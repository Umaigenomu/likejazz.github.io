...
#### 개선
한글 특성에 적합하도록 알고리즘을 개선했고(한국어 문장의 특성상 주어부에 추가 weighting 부여 등) n-grams에 대한 weight decaying 처리, 구문 학습, 상기 본문에 언급한 버그 포함, fastText의 버그라 예상되는 부분도 일부 패치했다. 이 내용들은 정리해서 PR을 보내 fastText upstream에 반영되도록 할 생각이다.
