...
#### n-grams
context 중 target 위치는 0으로 만들며 이를 `<PLACEHOLDER>`로 정한다. ngram 확장시에는 `<PLACEHOLDER>` 위치도 함께 확장을 한다. 만약 `-wordNgrams`가 3이라면 자기 자신 A를 포함해 (A,B), (A,C) 두 개를 더 추가하며 확장은 뒤로만 하되 이전 단어는 포함 하지 않는다. 또한 `-dropoutK`에 해당하는 토큰은 확장에서 배제한다. 그런데 ngram 확장이 (0,2), (0,6) 일때 마지막 (0,6)이 (2,6)과 같은 값을 갖는다. 이는 hash collision으로 일종의 버그로 추측된다.
