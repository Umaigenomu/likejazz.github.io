...
`MAXLEN=7`, `len(chars)=12`이므로 앞서 언급한 바와 같이 입력값은 `(None, 7, 12)`형태의 원-핫 벡터이며, 이를 64개의 LSTM을 이용해 인코딩한다. 각 중간층의 히트맵을 출력하여 각 레이어간 값의 변화를 시각적으로 표현해보도록 한다. 64개의 LSTM 결과를 8x8 히트맵으로 표현하면 아래와 같다.
...
히트맵이 길쭉해서 이상해 보이지만 y축에 따른 x의 값이 모두 동일하며 같은 값이 네 번 반복됨을 확인할 수 있다.
...
다른 부분은 결과가 명확한데, 3번째 위치가 조금 애매하다. x축 2번째 부터 0이 시작되므로 출력값은 `1004`가 될 확률이 가장 높지만 `1014`도 가능성이 높다.
...
히트맵은 seaborn으로 표현했다. seaborn에는 히트맵 기능이 포함되어 있어 매우 쉽게 만들 수 있다. 이외에도 Keras의 각 가중치를 아래와 같이 출력할 수 있다. 각각의 레이어가 학습이 잘 되었는지, 한 쪽으로 saturating 되진 않는지 확인하는 용도로 매우 유용하다.
...
binary cross entropy의 수식을 보면 negative를 1-true로 학습하는데, 나머지 모든 negative에 대한 $$\sum$$이 아니다. binary classification이라면 positive 외에는 모두 negative로 볼 수 있지만 multi-class는 그렇지 않다. Keras에서는 binary에 대한 metric이 정의되어 있지 않으며 아래 코드와 같이 metric을 보정해야 한다. 그 결과 binary cross entropy는 학습 성능에서 categorical cross entropy와 차이가 없음을 확인할 수 있다.
