...
한 가지 재밌는 점은 입력값을 뒤집는<sup>reverse</sup> 부분인데, 이는 LSTM의 입력값 처리 결과와 출력값 처리 결과를 보다 가까운 거리에 두어 long term dependencies 문제를 완화한다. (Sutskever et al., 2014)[^fn-2] 실제로 학습 결과, 입력 값을 뒤집지 않은 경우에 비해 약간의 개선 효과가 있었다.
