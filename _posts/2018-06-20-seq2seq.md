...
한 가지 재밌는 점은 입력값을 뒤집는<sup>reverse</sup> 부분인데, 이는 LSTM이 올바른 예측을 학습하도록 short term dependencies에 도움을 준다고 한다. (Sutskever et al., 2014)[^fn-2] 성능 향상에 도움이 된다고 알려져 있으며 실제로 학습 결과, 입력 값을 반전하지 않은 경우에 비해 약간의 개선 효과가 있었다.
...
binary cross entropy의 수식을 보면 negative를 1-true로 학습하는데, 나머지 모든 negative에 대한 $$\sum$$이 아니다. binary classification이라면 positive 외에는 모두 negative로 볼 수 있지만 multi-class는 그렇지 않다. Keras에서는 binary에 대한 metric이 정의되어 있지 않으며 아래 코드와 같이 metric을 보정해야 한다. 그 결과 binary cross entropy는 학습 성능에 도움이 되지 않음을 확인할 수 있다.
...
## 일반적인 형태
[Keras 공식 블로그에서도 소개](https://blog.keras.io/a-ten-minute-introduction-to-sequence-to-sequence-learning-in-keras.html)한 바 있는 Seq2Seq의 일반적인 형태는 좀 더 복잡하다. input과 output이 모두 가변 길이 이고, sequences 뿐만 아니라 states도 함께 사용하며, 학습과 추론의 입출력이 조금 다르다.
### 학습
<img width="80%" src="https://user-images.githubusercontent.com/1250095/42202348-fd526dae-7ed5-11e8-8015-76ebf2c545ac.png">
인코더의 states. 즉, hidden state와 cell state를 디코더의 입력으로 학습한다. 단, 디코더의 states는 사용하지 않는다.

### 추론
<img width="80%" src="https://user-images.githubusercontent.com/1250095/42202355-03059686-7ed6-11e8-9047-e472e7c25a1f.png">
인코더의 states는 첫 글자에 대해서만 입력으로 하고, 이후에는 디코더의 states를 다음 글자에 대한 입력으로 하면서 EOF에 도달할때 까지 추론을 수행한다.

## 코드
- [addition_rnn.py](https://github.com/likejazz/jupyter-notebooks/blob/master/deep-learning/addition_rnn.py)
- [lstm-seq2seq](https://github.com/likejazz/jupyter-notebooks/blob/master/deep-learning/lstm-seq2seq.py)
