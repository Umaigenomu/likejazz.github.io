...
<small>
*2018년 12월 22일 Sentence Representation 추가*  
*2018년 12월 17일 초안 작성*  
</small>
...
이 글에서는 BERT의 특징과 구조를 하나씩 살펴보고 한국어로 서비스 적용을 위해 실험을 진행한 부분까지 자세히 살펴보면서 BERT를 파헤쳐보도록 한다. 이 글을 작성하는데는 [Dissecting BERT 시리즈](https://medium.com/dissecting-bert)에 매우 큰 영감을 받았고, 이 글의 많은 부분에 차용했음을 일러둔다.
...
아래는 [카카오 브레인 이동현님의 PyTorch 구현](https://github.com/dhlee347/pytorchic-bert/blob/master/models.py#L123-L133)으로 코드가 매우 깔끔하여 읽기 쉽다. 마찬가지로 각 포지션별 Fully-connected로 처리한 후 다음 스텝에서 Residual로 연결한다.
...
[bert-as-service](https://github.com/hanxiao/bert-as-service)를 만든 Han Xiao라는 분이 효율적인 Sentence Representation을 위해 다양한 실험을 했다. BERT는 fine-tuning 하지 않고도 ELMo처럼 fixed feature vectors를 추출하여 pre-trained contextual embeddings를 활용할 수 있는데, 이를 문장 단위로 mean을 취하여 Sentence Representation으로 간주했다. 실제로 이 방식은 최근까지 여러 연구와 논문에서 주로 사용되는 Representation이지만 BERT 논문 저자는 [이 방식에 매우 부정적인 견해](https://github.com/google-research/bert/issues/164)를 보인다.
> we never suggested that this will generate meaningful sentence representations. 

실제로 BERT의 경우 Classification을 위한 `[CLS]` 벡터를 따로 갖고 있으며, 각각의 벡터가 토큰의 의미를 정확히 1:1로 표현한다고 보기는 어렵다. 따라서 분류 문제에서 어떤 레이어가 좋은 Representation인지 차원 축소를 통해 시각화를 진행했고, 결과는 아래와 같다.

<img src="https://github.com/hanxiao/bert-as-service/raw/master/.github/pool_mean.png?raw=true" width="100%">[^fn-10]

BERT-Base 모델의 인코더 블럭은 12개이고, 각각의 출력 결과를 차례대로 시각화 했다. `pooling_layer=-1`는 출력에 가장 가까운 레이어이고, 지나치게 타겟에 편향된 결과를 보인다. 만약 fine-tuning을 하지 않고 pre-trained 모델을 그대로 사용한다면 결코 좋은 Representation으로 볼 수 없다. 오히려 입력에 가장 가까운 첫 번째 레이어인 `pooling_layer=-12`가 워드 임베딩과 가장 유사한 모습을 보였고, 이를 이용한다면 보다 좋은 분류 성능을 얻을 수 있을 것이다.
...
<img src="https://user-images.githubusercontent.com/1250095/50079301-9ac61480-022c-11e9-9d15-6663e2e49ab7.jpeg" width="80%">
...
#### IMDB
IMDB Sentimental Analysis의 경우 더 좋은 성능을 보인다. CNN, RNN등을 아무리 활용해도 85% 수준에 머무르고 조금이라도 성능을 더 높이는 일은 매우 어렵고 까다로운 일인데 BERT는 단순히 pre-trained 모델을 fine-tuning 하는 것 만으로도 93.3%에 달하는 결과를 얻을 수 있었다.

<img src="https://user-images.githubusercontent.com/1250095/50357199-75246c80-0598-11e9-8bae-f147e4b03ab7.jpeg" width="80%">

결과 또한 valid, test의 차이가 거의 없을 정도로 안정적이다. 현재 [NLP-progress에 등록된 state-of-the-art](http://nlpprogress.com/english/sentiment_analysis.html)는 다름 아닌 Sebastian Ruder가 직접 참여한 ULMFiT(Howard et al., 2018) 모델로 95.4%를 기록하고 있는데, BERT의 경우 아무런 튜닝 작업 없이도 거의 state-of-the-art에 근접한 매우 뛰어난 성능을 확인할 수 있다.
...
#### 한국어 적용
한국어로 서비스를 구현하면서 벡터의 단순 sum/mean을 넘어 보다 다양한 변형으로 실험을 진행하고 품질을 확인했다.
...
여기서 구현하고자 하는 모델은 입력 문장에 대한 각 문장의 유사도 순위를 판별하는 모델이며, 앞서 `[CLS]` 벡터를 이용하거나 pooling layer의 변경, 벡터 sum/mean 방식의 변형 등 다양한 방안을 고민할 수 있다. 여기서는 baseline으로 zero padding에 대한 벡터는 제외하고, BM25로 [Weighted Sum](#weighted-sum)을 구현해 중요 벡터에 가중치를 높이는 방식으로 아래와 같은 품질을 얻을 수 있었다.

<img src="https://user-images.githubusercontent.com/1250095/50047189-1a9c9380-00f4-11e9-87e0-98f2260f314d.png">

테스트셋으로 정량적인 평가가 가능하도록 해 지속적인 실험 개선이 가능하게 구현하였고, fine-tuning 등을 진행해 보다 효율적인 Sentence Representation을 위한 다양한 실험을 진행해 본다면 훨씬 더 좋은 결과를 얻을 수 있을 것으로 기대한다.
