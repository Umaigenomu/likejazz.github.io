...
대신 토큰의 상대적 또는 절대적 위치에 대한 정보를 주입하는데 이를 Positional Encoding 이라 하며 앞서 Transformer 모델 그림에 잘 나와 있다. 이는 사인 함수<sup>sinusoid function</sup>를 이용한 아래 수식의 결과를 더해 네트워크가 토큰의 상대적 위치와 관련한 정보를 학습할 수 있게 한다.
...
헤드를 계산하는 방법을 보다 자세히 살펴보면 Scaled Dot-Product Attention 결과를 앞서 언급한 대로 Concatenates 하는 형태를 띈다.
...
이 처럼 커널 사이즈가 클때 인코더 블럭이 반복되면 주변 컨텍스트 정보를 점점 더 모아올 수 있어 성능에 도움이 된다고 얘기를 한다.
...
논문 구현에는 Sentence Pair Classification에, [학습](#학습) 항목에서 확인한 바 있는 첫 번째 레이어 `[CLS]`의 벡터로 판별하도록 되어 있고 동일한 구현으로 아래와 같은 결과를 얻을 수 있었다.
...
### 서비스
BERT를 서비스로 구현하기 위해 여기서는 ZeroMQ를 이용해 웹서비스 형태로 구현한 [bert-as-service](https://github.com/hanxiao/bert-as-service)를 이용해 데모를 구현했다. 초기 버전의 경우 몇몇 심각한 버그가 있었고 때문에 고생을 많이 하였으나 [일부 버그는 직접 패치를 제출](https://github.com/hanxiao/bert-as-service/pull/101)했고, 원저자의 적극적인 피드백으로 빠르게 패치될 수 있었다. 매우 활발하게 개선되고 있어 앞으로가 기대되는 프로젝트다. 배치 사이즈를 256 정도로 설정했을때 Tesla V100 GPU에서 Inference가 초당 1,000 TPS에 달해 서비스를 하기에도 충분한 속도를 지니고 있다.

bert-as-service는 Sentence Representation으로 모든 토큰 벡터의 Sum을 취하도록 구현되어 있으며 이 방식은 실제로 최근까지 여러 연구와 논문에서 주로 사용되는 Representation이다. 그러나 논문 저자는 [이 방식에 매우 부정적인 견해](https://github.com/google-research/bert/issues/164)를 보인다.
> we never suggested that this will generate meaningful sentence representations.

실제로 BERT의 경우 Classification을 위한 `[CLS]` 벡터를 따로 갖고 있으며, 각각의 벡터가 토큰의 의미를 정확히 1:1로 표현한다고 보기는 어렵다. 이 때문에 서비스 구현시에는 단순 Sum을 넘어 보다 다양한 실험을 진행해 품질을 확인했다.

#### 한국어 적용

얼마전 [카카오 한글 형태소 분석기](https://github.com/kakao/khaiii)를 공개한 우리팀 카카오 임재수님의 도움으로 BERT의 한국어 적용을 진행했다. 학습 데이터셋으로 세종 코퍼스를 카카오에서 패치한 문종 코퍼스를 포함해 다양한 한글 데이터셋의 형태소 분석 결과를 준비했고, 처음부터 Pre-train을 진행했다.

입력 문장에 대한 각 문장의 유사도 순위를 판별하는 모델 구현을 시도했으며, 앞서 `[CLS]` 벡터를 이용하는 등 여러가지 방법을 고민할 수 있다. 여기서는 제로 패딩에 대한 벡터를 제외하고, BM25로 [Weighted Sum](#weighted-sum)을 구현하여 중요 벡터에 대해 가중치를 높이는 방식으로 아래와 같은 품질을 얻을 수 있었다.

<img src="https://user-images.githubusercontent.com/1250095/50047189-1a9c9380-00f4-11e9-87e0-98f2260f314d.png">

테스트셋으로 정량적인 평가를 하면서 Fine-tuning을 더 진행해보고 보다 효율적인 Sentence Representation을 위한 다양한 실험을 진행해 본다면 훨씬 더 좋은 결과는 물론 좋은 서비스로 연결할 수 있을 것으로 기대한다.
