...
BERT-Base 모델의 인코더 블럭은 12개이며 각각의 출력 결과를 차례대로 시각화 했다. `pooling_layer=-1`는 출력에 가장 가까운 레이어이고, 지나치게 타겟과는 편향된 결과를 보인다. 만약 fine-tuning을 하지 않고 pre-trained 모델을 그대로 사용한다면 좋은 Representation으로 보기 어렵다.

<img src="https://user-images.githubusercontent.com/1250095/50369600-a2ded500-05db-11e9-8edf-e3126941d58b.png" width="80%">[^fn-3]

NER 태스크의 경우 논문에서 마지막 4개 레이어를 concatenates 했을때 fine-tuned 모델과 거의 차이가 없는 결과를 보였다고 보고한바 있다. 여기서는 입력에 가장 가까운 첫 번째 레이어 `pooling_layer=-12`가 워드 임베딩에 가장 유사한 모습을 보였고, 이를 이용한다면 가장 좋은 분류 성능을 얻을 수 있을 것으로 보인다.
...
#### IMDB
IMDB Sentimental Analysis의 경우 매우 좋은 결과를 확인할 수 있었다. CNN, RNN을 아무리 잘 구성해도 85% 수준에 머무르고 약간 더 성능을 높이는 일도 매우 어렵고 까다로운 일인데 BERT는 단순히 pre-trained 모델을 fine-tuning 하는 것 만으로도 93.3%에 달하는 결과를 얻을 수 있었다.
...
실험은 정량적인 평가가 가능하도록 테스트셋을 구성해 지속적인 실험 개선이 가능하도록 했다. fine-tuning 등을 진행해 보다 효율적인 Sentence Representation을 위한 다양한 실험을 진행해 본다면 훨씬 더 좋은 결과를 얻을 수 있을 것으로 기대한다.
