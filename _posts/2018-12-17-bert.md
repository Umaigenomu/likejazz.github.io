...
형태소 분석에는 카카오 내부에서 사용중인 형태소 분석기 DHA2를 사용했다. Pre-trained 모델의 성능에 따라 최대 2% 까지 성능 차이가 나기도 했으며, Pre-trained 코퍼스는 대부분이 문어체로, 평가에 사용된 리뷰의 경우 구어체라 OOV가 많이 나왔음에도 불구하고 매우 좋은 결과를 확인할 수 있었다. 만약 문어체 모델을 사용했다면 훨씬 더 좋은 결과가 나올거라 예상된다.
