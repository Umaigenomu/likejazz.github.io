...
[dtreeviz](https://explained.ai/decision-tree-viz/index.html)는 보다 직관적으로 보여준다. 동일한 트리를 아래와 같이 훨씬 해석하기 쉽도록 보여준다.
...
타이타닉의 경우 Decision Tree는 80.9%, Random Forests는 81.6% 성능을 보인다. 타이타닉 자체의 데이터 변별력이 부족하므로 큰 의미는 없으나 [전처리를 통해 92%까지](https://towardsdatascience.com/predicting-the-survival-of-titanic-passengers-30870ccc7e8) 높일 수 있다.
...
### Interpretable ML
#### 책
아래 두 권을 추천한다. 원고를 오픈한 상태로 여전히 작성 중이다.
...
SHAP은 Grid Search와 유사하게 동작하며, 이를 통해 SHAPley Values를 추출한 후 [React로 만든 js 라이브러리](https://github.com/interpretable-ml/iml)를 이용해 시각화 한다. 
