...
# 프로그래머, 수학으로 생각하라
CS의 기본적인 알고리즘을 특유의 일본 책 답게 매우 알기쉽게 잘 설명한다. 대부분의 알고리즘 책들은 자료구조에 관한 내용이 대부분인데 반해 이 책은 자료구조에 관한 내용 전혀 없이 수학적 알고리즘에 관한 내용만을 다룬다. 개정판에는 머신러닝에 관한 내용이 추가되었으나 이 때문에 굳이 새로 구입할 필요는 없다.
