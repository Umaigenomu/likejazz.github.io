...
| 표현 | 전개 |
| ------ | --- |
| exp(x) | <img src="http://chart.apis.google.com/chart?cht=tx&chl=e^x" /> |
| ln(x)  | <img src="http://chart.apis.google.com/chart?cht=tx&chl=log_e(x)" /> 자연로그<sup>natural logarithm</sup> |
| log(x) | <img src="http://chart.apis.google.com/chart?cht=tx&chl=log_{10}(x)" /> 상용로그<sup>common logarithm</sup> |

e(자연상수, 오일러의 수, 네이피어 상수) 값은 [x = 0 일때 기울기 45도](http://www.librow.com/articles/article-11/appendix-a-20)

[다양한 로그 그래프](http://mathonweb.com/help_ebook/html/functions_3.htm)

곱이 아닌 합으로 계산 가능한 로그의 성질 <img src="http://chart.apis.google.com/chart?cht=tx&chl=\log{xy}=\log{x}%2B\log{y}" />을 이용해 상용로그가 등장. 컴퓨터가 발달한 오늘날엔 더 이상 유용하지 않지만 개념적 이유로 여전히 과학에서 핵심적인 역할을 하고 있다.

<img src="http://chart.apis.google.com/chart?cht=tx&chl=10^x=4" /> 로그 계산법 정리. <img src="http://chart.apis.google.com/chart?cht=tx&chl=5^{\frac{2}{3}}=\sqrt[3]{5^2}{\approx}2.924" /> 이것도 같이 정리 필요.  
[2018학년도 대학수학능력시험 수학 4점 문제 해설](http://blog.naver.com/wowhd93/221148903996) 정리 유형 참고
...
베이즈 통계학
- DNA 프로파일링  
approximation algorithm을 이용했다.
- 소인수분해의 어려움을 이용한 암호화 알고리즘  
리만 가설의 해법을 연구한다는 에피소드
- 점 잇기의 수학  
소셜 네트워크를 그래프 알고리즘으로 분석
- 게임이론과 위험분석  
죄수의 딜레마. 협력의 메커니즘.
- 법정에 선 수학  
