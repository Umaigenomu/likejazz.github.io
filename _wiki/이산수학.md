...
- [Boolean Algebra, Logic, Set Theory](#boolean-algebra-logic-set-theory)
    - [논리 <sup>logic</sup>](#논리-suplogicsup)
    - [부울 대수 <sup>boolean algebra</sup>](#부울-대수-supboolean-algebrasup)
        - [드모르간 법칙](#드모르간-법칙)
    - [카르노 맵 <sup>Karnaugh map</sup>](#카르노-맵-supkarnaugh-mapsup)
- [순열, 조합](#순열-조합)
- [재귀](#재귀)
- [그래프](#그래프)
...
- [귀납, 연역](#귀납-연역)
    - [수학적 귀납법 <sup>mathematical induction</sup>](#수학적-귀납법-supmathematical-inductionsup)
- [오토마타](#오토마타)
    - [튜링 머신](#튜링-머신)
    - [촘스키 계층 <sup>Chomsky Hierarchy</sup>](#촘스키-계층-supchomsky-hierarchysup)
    - [SR 플립플롭](#sr-플립플롭)
...
결국 flowchart가 FSM이 아닌가 하는 생각을 한적이 있는데, [A flowchart is not a state machine(PDF)](http://www.stateworks.com/active/download/TN9-Flowchart-is-not-State-Machine.pdf) 글에서 그렇지 않음을 예제를 통해 잘 설명한다.
