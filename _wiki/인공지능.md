...
Decision Tree에서 Entropy를 낮추고 Information Gain을 가장 높일 수 있는 Greedy로 동작하는 것과 유사해 보인다. 모든 것이 동일할 경우 후진연쇄 방법이 구현하기 쉽다. 전문가 시스템이 등장했던 1970년대는 아직 ID3 알고리즘(1986)이 등장하기 이전이다.

# 에이전트
에이전트를 용어가 사용되기 시작한 것은 1980년 MIT에서 개최된 분산 인공지능에 관한 첫 번째 워크숍 이후.
