...
아래는 IRIS 데이타셋으로 random forest로 100개 만들었고[^12] 그 중 하나를 dot export[^14] 하여 graphviz viewer[^13]로 확인한 모습.
...
`samples`는 해당 decision의 샘플 수, `[Setosa, Versicolor, Virginica]` 결과에 해당하는 값이 `values = [63,41,46]`, `feature_names`와 `class_names`를 부여하지 않았기 때문에 라벨링이 되어 있지 않음. decision trees는 value의 합이 samples의 수와 일치하지만 이 경우는 random forest라 일치하지 않음. 아래는 titanic kaggle 데이타를 decision trees로 표현. random forest가 아니기 때문에 values의 합이 samples의 수와 일치. 라벨링을 했고 `filled=True`로 처리하여 훨씬 알아보기 좋음.

<img src="http://docs.likejazz.com/public/notebooks/titanic.png" width="100%" />

[titanic.ipynb](https://nbviewer.jupyter.org/github/likejazz/likejazz.github.io/blob/master/public/notebooks/titanic.ipynb)

decision trees는 80.9%, random forest는 81.6%. 데이타의 변별력이 부족하므로 큰 차이는 없음.
