...
# 활용 예제
## 뉴스 카테고리 분류
Kaggle의 뉴스 데이타 이용, 뉴스 제목을 이용해 카테고리를 분류하는 실험 진행. one-hot vector라 decision tree 룰을 수정하는게 의미 없고, random forest 또한 화이트박스 이지만 룰을 수정하기가 쉽지 않다. multinomial naive bayes에서 90%가 넘는 가장 높은 정확도 기록[^15] 실행 속도도 NB가 가장 빠르다.
